  ...
  await resend.emails.send({
    ...
    html: `
      ...
      <div style="padding:28px 24px; font-size:14px; line-height:1.7; color:#111111;">
        <p style="margin:0 0 14px;">Hi ${captainName || 'Captain'},</p>
...
async function sendCaptainSubscriptionEmail(vesselId: string) {
  ...
  await resend.emails.send({
    ...
    html: `
      ${BRAND_HEADER}
      <div style="padding:28px 24px; font-size:14px; line-height:1.7; color:#111111;">
        <p style="margin:0 0 14px;">Hi ${captain.name || 'Captain'},</p>
...
async function sendCrewJoinedEmail(userRecord: any, vesselName: string) {
  await resend.emails.send({
    ...
    to: userRecord.email,
    replyTo: 'support@nautical-ops.com',
    subject: `You're in \u2014 welcome to ${vesselName}`,
    html: `
      ${BRAND_HEADER}
      <div style="padding:28px 24px; font-size:14px; line-height:1.7; color:#111111;">
        <p style="margin:0 0 14px;">Hi ${userRecord.name || 'there'},</p>
        <p style="margin:0 0 14px;">You've just joined <strong>${vesselName}</strong> on Nautical Ops. Everything you need day to day \u2014 tasks, watch schedules, hours of rest, safety info \u2014 is right there in the app.</p>
  ...
  try {
    ...
    if (payload.table === 'users') {
      const firstVesselJoin =
        payload.record?.vessel_id &&
        (payload.type === 'INSERT' || !payload.old_record?.vessel_id);
